	...
	if (mem_avail) {
		...
		for(i = 0; i < NUM_PAGES; i++){
			if(_mem_stat[i].proc == 0){
				...
				addr_t cur_page_vir_addr = (num_alloc_pages << OFFSET_LEN) + ret_mem;
				int found = 0;

				int j;
				for(j = 0; j < seg_table->size; j++){
					if( seg_table->table[j].v_index == get_first_lv(cur_page_vir_addr) ){
						struct page_table_t * cur_page_table = seg_table->table[j].pages;

						cur_page_table->table[cur_page_table->size].v_index = get_second_lv(cur_page_vir_addr);
				...
				if(!found){
					seg_table->table[seg_table->size].v_index = get_first_lv(cur_page_vir_addr);
					seg_table->table[seg_table->size].pages = (struct page_table_t *)malloc(sizeof(struct page_table_t));

					seg_table->table[seg_table->size].pages->table[0].v_index = get_second_lv(cur_page_vir_addr);
		...
		for(i = 0; i < page_table->size; i++){
			if(page_table->table[i].v_index == get_second_lv(address)){
				...
				if(translate(address, &physical_addr, proc)){
					...
					addr_t cur_page_vir_addr = (num_free_pages << OFFSET_LEN) + address;

					do{
						_mem_stat[p_index].proc = 0;
						//_ram[p_index << OFFSET_LEN] = 0;

						int found = 0;

						int k;
						for(k = 0; k < proc->seg_table->size && !found; k++){
							if( proc->seg_table->table[k].v_index == get_first_lv(cur_page_vir_addr) ){
								int l;
								for(l = 0; l < proc->seg_table->table[k].pages->size; l++){
									if(proc->seg_table->table[k].pages->table[l].v_index
									   == get_second_lv(cur_page_vir_addr)){
										int m;
										for(m = l; m < proc->seg_table->table[k].pages->size - 1; m++)
